...
# Fix DPI scaling on Windows for crisp, properly sized UI
try:
    import ctypes
...
def is_duplicate(file_name):
    # Detect duplicates like "Discord (2).lnk"
...
def list_uwp_apps():
    # Use PowerShell Get-StartApps to get installed Start menu apps
    ps_list_apps = (
    ...
    try:
        ...
        print(f"Failed to list UWP apps: {e}")
        return []
...
def load_layout():
    ...
    with open(LAYOUT_FILE, "r", encoding="utf-8") as f:
        layout = json.load(f)
    # Filter duplicates in classic list
...
class TaskbarBackupApp:
    ...
    def show_saved(self):
        # Toggle saved layout window on/off
